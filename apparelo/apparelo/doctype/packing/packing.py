...
class Packing(Document):
	...
	def create_boms(self, input_item_names, variants, colour, attribute_set=None, item_size=None, piece_count=None, final_item=None, final_process=None):
		...
		if self.is_combined:
			create_intermediate_item_template(final_item)
			intermediate_variants = create_intermediate_variant(final_item, colour, item_size)
			for intermediate_variant in intermediate_variants:
				intermediate_variant_doc = frappe.get_doc("Item", intermediate_variant)
				intermediate_variant_attr = get_attr_dict(intermediate_variant_doc.attributes)
				item_list_ = []
				for items in input_item_names:
					input_item_doc=frappe.get_doc("Item",items)
					input_attr = get_attr_dict(input_item_doc.attributes)
					variant_of = input_item_doc.variant_of
					for item_mapping in self.combined_item_mapping:
						if item_mapping.intermediate_item == variant_of and item_mapping.intermediate_item_colour in input_attr["Apparelo Colour"] and item_mapping.final_item_colour in intermediate_variant_attr["Apparelo Colour"]:
							if input_attr["Apparelo Size"] == intermediate_variant_attr["Apparelo Size"]:
								item_list_.append({
									"item_code": items,
									"uom": "Nos"
								})
				...
				existing_bom = frappe.db.get_value(
					'BOM', {'item': intermediate_variant}, 'name')
				if not existing_bom:
					new_bom = frappe.get_doc({
						"doctype": "BOM",
						"currency": get_default_currency(),
						"uom": "Nos",
						"item": intermediate_variant,
			...
			for variant in variants:
				...
				for intermediate_variant in intermediate_variants:
					...
					for size in item_size:
						if size in variant_attr["Apparelo Size"] and size in intermediate_variant_attr["Apparelo Size"]:
							items_.append({
								"item_code": intermediate_variant,
								"uom": "Nos",
								"bom_no": get_item_details(intermediate_variant).get("bom_no")
							})
				intermediate_existing_bom = frappe.db.get_value(
					'BOM', {'item': variant}, 'name')
				if not intermediate_existing_bom:
					bom = frappe.get_doc({
						"doctype": "BOM",
						"currency": get_default_currency(),
						"uom": "Nos",
					...
					boms.append(intermediate_existing_bom)
...
def create_intermediate_item_template(final_item):
	if not frappe.db.exists("Item", final_item+" Intermediate Cloth"):
		frappe.get_doc({
			"doctype": "Item",
			"item_code": final_item+" Intermediate Cloth",
			"item_name": final_item+" Intermediate Cloth",
			"description": final_item+" Intermediate Cloth",
			"item_group": "Sub Assemblies",
			"stock_uom": "Nos",
			"has_variants": "1",
			"variant_based_on": "Item Attribute",
...
def create_intermediate_variant(final_item, colours, size):
	...
	attribute = {"Apparelo Size": size, "Apparelo Colour": colours}
	intermediate_variants.extend(create_variants(
		final_item+" Intermediate Cloth", attribute))
	return intermediate_variants