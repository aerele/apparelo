...
class DC(Document):
    def on_submit(self):
        ...
        msgprint(_("{0} created").format(comma_and(
            """<a href="#Form/Stock Entry/{0}">{1}</a>""".format(stock.name, stock.name))))
...
def create_purchase_order(self, abbr):
    ...
    for item_ in self.return_materials:
        dc_items.append({"item_code": item_.item_code, "schedule_date": add_days(
            nowdate(), 7), "qty": item_.quantity})
    for item in self.items:
        supplied_items.append({"main_item_code": item.item_code, "rm_item_code": item.item_code,
                               "required_qty": item.quantity, "reserve_warehouse": f'{self.lot} - {abbr}'})
    po = frappe.get_doc({
...
def create_purchase_receipt(self, po, abbr):
    ...
    for item in po.items:
        item_list.append({"item_code": item.item_name, "qty": item.qty, "bom": item.bom,
                          "schedule_date": add_days(nowdate(), 7), "warehouse": f'{self.lot} - {abbr}'})
    frappe.get_doc({
        "docstatus": 1,
        "supplier": self.supplier,
        "set_warehouse": f'{self.lot} - {abbr}',
...
def create_stock_entry(self, po, stock_entry_type, abbr):
    ...
    for item in self.return_materials:
        item_list.append({"allow_zero_valuation_rate": 1, "s_warehouse": f'{self.lot} - {abbr}',
                          "t_warehouse": f'{self.supplier} - {abbr}', "item_code": item.item_code, "qty": item.quantity})
    if po == "":
        se = frappe.get_doc({
            "docstatus": 1,
...
def get_ipd_item(doc):
    ...
    process_index = ''
    item_production_detail = frappe.db.get_value(
        "Lot Creation", {'name': lot}, "item_production_detail")
    ipd = frappe.db.get_value('IPD Item Mapping', {
                              'item_production_details': item_production_detail}, 'name')
    ...
    for item in ipd_item.item_mapping:
        if process == 'Knitting':
            ...
            items.append({"item_code": item.input_item,
                          "available_quantity": data[0]['bal_qty'], 'uom': data[0]['stock_uom']})
    ...
    for item in ipd_item.item_mapping:
        for index in input_indexs:
            if str(item.ipd_process_index) == index:
                ...
                items.append(
                    {"item_code": item.item, "available_quantity": data[0]['bal_qty'], 'uom': data[0]['stock_uom']})
    return items
        ...
        for item in bom_.items:
            for data in items:
                ...
                if data.get('item_code') == item.item_code:
                    ipd_name = frappe.db.get_value(
                        "Lot Creation", {'name': doc.lot}, 'item_production_detail')
                    ipd_item_map = frappe.get_doc(
                        "IPD Item Mapping", {'item_production_details': ipd_name})
                    ...
                    return_materials.append(
                        {"item_code": bom_.item, "uom": bom_.uom, "quantity": per_item/item.qty, "description": description})
