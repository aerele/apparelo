...
class Knitting(Document):
  ...
  def create_variants(self, input_item_names):
    ...
    if self.is_striped:
      colour = ''
      for shade in attribute_set['Yarn Shade']:
        colour += shade+","
      # attribute_set["Apparelo Colour"] = [colour[:-1]]
      ...
      if not colour[:-1] in colours:
        ...
        item_attribute.save()
      template = "Striped Knitted Cloth"
    else:
      template = "Knitted Cloth"
    variants = create_variants(template, attribute_set)
    for dia in attribute_set["Dia"]:
      for variant in variants:
        if dia in variant:
          if not dia+" Dia" in variant:
            # for colour in attribute_set['Apparelo Colour']:
            #   print(colour,variant)
            #   if colour.upper() in variant:
            #     variant=variant.replace(colour.upper(),"")
            #     print(variant,"11111111111111111111111111111111111111111111111")
            hash_ = hashlib.sha256(variant.replace(
  ...
  def create_boms(self, input_item_names, variants, attribute_set, item_size, colour, piece_count):
    ...
    for item in input_items:
      ...
      for attribute_values in args_set:
        ...
        if self.is_striped:
          # colour = ''
          # for shade in [attribute_values['Yarn Shade']]:
              
          #     colour += shade+","
          # attribute_values["Apparelo Colour"] = colour[:-1]
          variant = get_variant("Striped Knitted Cloth", args=attribute_values)
          if variant in variants:
            item_list = []
            for yarn_ratio in self.apparelo_yarn_ratio:
              item_list.append({"item_code": yarn_ratio.yarn,
                               "qty": yarn_ratio.quantity, "uom": self.input_uom, "rate": 0.0})
            ...
            existing_bom_name = frappe.db.get_value(
                'BOM', {'item': variant, 'docstatus': 1, 'is_active': 1}, 'name')
            if not existing_bom_name:
              ...
              if similar_diff:
                ...
                frappe.throw(
                    _("Active BOM with different Materials or qty already exists for the item {0}. Please make these BOMs inactive and try again.").format(variant))
          else:
            frappe.throw(
                _("Unexpected error while creating BOM. Expected variant not found in list of supplied variants"))
...
def create_striped_item_template():
    if not frappe.db.exists("Item", "Striped Knitted Cloth"):
        frappe.get_doc({
            "doctype": "Item",
            "item_code": "Striped Knitted Cloth",
            "item_name": "Striped Knitted Cloth",
            "description": "Striped Knitted Cloth",
            "item_group": "Sub Assemblies",
            "stock_uom": "Kg",
            "has_variants": "1",
            "variant_based_on": "Item Attribute",
